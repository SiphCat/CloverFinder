-- Map + leaderboard performance helpers.
--
-- Run the ENTIRE file in Supabase → SQL Editor (Cmd+A in the file, then Run).
-- Do not run leaderboard.sql alone — use setup_leaderboard.sql instead if you only need the table + rankings.
-- For the full app (storage, badges scan, clovermedia), run cloverfinder_finds_badges_media.sql.

-- ─── user_badges (required for leaderboard) ────────────────────────────────
...
-- ─── Map pin index ─────────────────────────────────────────────────────────
create index if not exists finds_lat_lng_idx on public.finds (lat, lng);

-- ─── Leaderboard (top finders by verified badge count) ─────────────────────
